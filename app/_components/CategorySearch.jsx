    ...
    <div>
      <div className="items-center flex flex-col mb-10">
        ...
        <div className="flex w-full max-w-sm items-center space-x-2 mt-4">
          ...
          <Button type="submit">
            <Search className="h-4 ml-1" /> Search
          </Button>
        </div>
      </div>
        ...
        {categoryList.length > 0
          ? categoryList.map((item, index) => (
              ...
              <Link href={'/search/' + item.attributes?.Name}
                key={index}
                className="flex flex-col bg-red-100 items-center gap-2 p-2 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6"
              >
                <Image
                  src={
                    item.attributes?.ImageURL?.data[0]?.attributes?.formats
                      ?.medium?.url ||
                    item.attributes?.ImageURL?.data[0]?.attributes?.url
                  }
                  alt="icon"
                  height={40}
                  width={40}
          ...
          : [1, 2, 3, 4, 5, 6, 7].map((item, index) => (
              <div className=" h-[120px] bg-slate-600 w-[120px] animate-pulse"></div>
            ))}
      </div>
    </div>
  );
